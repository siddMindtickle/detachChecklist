...
export const createSyncActions = actionType => {
  const { TRIGGER, FULLFILL, REFRESH } = SyncActionsSuffix;
  return {
...
export const getLoadingActions = actionType => {
  const { SUCCESS, FAIL } = LodingActionSuffix;
  return {
...
export const createSyncActionFunctions = actionType => {
  const { TRIGGER, FULLFILL, REFRESH } = createSyncActions(actionType);
  return {
...
export const generateActionsAndFunctions = (actionType, { async } = {}) => {
  const { SUCCESS, FAIL, LOADING, TRIGGER, FULLFILL } = getLoadingActions(actionType);
  ...
  if (async) {
    const { LOADING: loading, SUCCESS: success, FAIL: fail } = generateAction(actionType, {
      async
    });
...
export const getActionObjects = actionTypes => {
  if (Array.isArray(actionTypes)) {
    const actionObjects = {};
    actionTypes.reduce((object, action) => {
